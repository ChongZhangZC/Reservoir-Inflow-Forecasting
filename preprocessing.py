...
''' 采用LSTM模型进行预测
预处理：
整个数据集按年度和季节分为 13 个数据⼦集。对每个⼦集分别构建相应的训练集和独
⽴测试集，⽐例为 9 : 1
'''
import pandas as pd
...
if __name__ == '__main__':
    ...
    data_2018_spring = './data/2018年春.xls'
    # 读取数据
    ...
    df_2018_spring = pd.read_excel(data_2018_spring, index_col='TIME')
    # 合并为一年
    ...
    # 绘制柘溪年度流量
    # df_2015['zhexi_in'].plot()
    # df_2016['zhexi_in'].plot()
    # df_2017['zhexi_in'].plot()
    # plt.title('zhexi inflow over years 2015 to 2017')
    # plt.show()

    # 训练
    dfs = [df_2015_spring,df_2015_summer,df_2015_autumn, df_2015_winter,
    df_2016_spring , df_2016_summer, df_2016_autumn,df_2016_winter,
           df_2017_spring, df_2017_summer, df_2017_autumn, df_2017_winter,
           df_2018_spring]
   ...
   # 模型参数
    ...
    epochs = 1000
    #
    x_train, y_train, x_test, y_test = load_data_from_df(df_2015_spring, use_arma_mode=False)
    ...
    # 训练
    losses = np.zeros(shape=epochs,dtype=np.float32)
